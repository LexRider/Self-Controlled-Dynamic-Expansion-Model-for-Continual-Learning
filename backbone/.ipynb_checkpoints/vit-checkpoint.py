...
class VisionTransformer(MammothBackbone):
    ...
    def forward(self, x: torch.Tensor, AB: dict = {}, returnt='features'):
        assert returnt in ('out', 'features', 'both', 'full', 'cls_tokens', '3features')
    
        # 处理 cls_tokens 模式
        if returnt == 'cls_tokens':
            cls_tokens = []
            features = self.forward_features(x, AB, return_all=True)
            for feature in features[:-1]:
                cls_token = feature[:, 0, :]
                cls_token = self.norm(cls_token)
                cls_tokens.append(cls_token.clone())
            return torch.stack(cls_tokens, dim=0)
    
        # 提取特征，是否获取所有层视 returnt 而定
        get_all = returnt in ['3features', 'full', 'both']
        x = self.forward_features(x, AB, return_all=get_all)
    
        if returnt == 'full':
            return x
    
        if returnt == '3features':
            feats_1 = x[-1][:, 0, :]
            feats_2 = x[-2][:, 0, :]
            feats_3 = x[-3][:, 0, :]
        ...
        if returnt == 'both':
            pooled_feat = self.forward_head(x[-1], pre_logits=True)
            feats_1 = x[-1][:, 0, :]
            feats_2 = x[-2][:, 0, :]
            feats_3 = x[-3][:, 0, :]
            return pooled_feat, (feats_1, feats_2, feats_3)
    
        # 默认行为：只提特征或输出分类
        pooled_feat = self.forward_head(x, pre_logits=True)
        if returnt == 'features':
            return pooled_feat
    
        return self.head(pooled_feat)  # returnt == 'out'
    ...
    #     :param x: 输入张量
    #     :param AB: LoRA-style 参数
    #     :param returnt: 控制返回的内容
    #     :return: 可返回最终分类输出、特征向量、多个层的特征等
    #     """
    #     assert returnt in ('out', 'features', 'both', 'full', 'cls_tokens', '3features')
    ...
    #     # 正常的 feature 提取逻辑
    #     x = self.forward_features(x, AB, return_all=(returnt in ['full', '3features']))
        
    #     if returnt == 'full':
    #         all_features = x
    #         x = x[-1]
        
    #     elif returnt == '3features':
    #         assert isinstance(x, list) and len(x) >= 3, "返回层数不足，无法提取 3 层特征"
            
    #         feats_1 = self.forward_head(x[-1], pre_logits=True)  # 最后一层
    #         feats_2 = self.forward_head(x[-2], pre_logits=True)  # 倒数第二层
    #         feats_3 = self.forward_head(x[-3], pre_logits=True)  # 倒数第三层
            
    #         return feats_1, feats_2, feats_3  # 直接返回 3 个特征

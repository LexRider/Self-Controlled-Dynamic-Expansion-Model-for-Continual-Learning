...
def save_samples_per_class(dataloader, num_classes, task_id, output_dir='output'):
    TINY_MEAN, TINY_STD = [0.4802, 0.4480, 0.3975], [0.2770, 0.2691, 0.2821]
    """
    从 dataloader 中选取每个类别的一个样本并将其保存到指定目录。

    Args:
        dataloader (DataLoader): PyTorch 的数据加载器。
        num_classes (int): 数据集中的类别数。
        task_id (int): 当前任务的编号，用于标识文件名。
        output_dir (str): 保存图片的目录路径，默认为 'output'。
    ...
    # 创建一个字典，用于存储每个类别的第一个样本
    class_samples = {}
    
    # 遍历 dataloader 以找到每个类别的第一个样本
    ...
    # 遍历每个类别，将对应的图片保存为文件
    for label, img in class_samples.items():
        # 定义保存图片的文件名，格式为 task_{task_id}_class_{label}.jpg
        file_name = f'task_{task_id}_class_{label}.jpg'
        file_path = os.path.join(output_dir, file_name)
        
        # 保存图片，先取消归一化，然后保存为jpg格式
        img = (img * torch.tensor(TINY_STD).view(3, 1, 1)) + torch.tensor(TINY_MEAN).view(3, 1, 1)  # 反归一化
        save_image(img, file_path, format='jpeg')

    print(f"所有类别的样本图片已保存到目录: {output_dir}")
        ...
        while True:
            ...
            if 'class-il' not in model.COMPATIBILITY and 'general-continual' not in model.COMPATIBILITY:
                ...
                if model.args.eval_future and k >= model.current_task:
                    ...
                    feats2 = model.net2(inputs, k)
            
                    feats = model.weight1 * feats1 + model.weight2 * feats2                    
...
def train(model: ContinualModel, dataset: ContinualDataset,
    ...
    with track_system_stats(logger) as system_tracker:
        ...
        for t in range(start_task, end_task):
            ...
            #################################################################################
            # save_samples_per_class(train_loader, num_classes=20, task_id=t, output_dir='output')
            ...
            # 激活当前任务输出头，屏蔽其它输出头的输出，同时增加一个resnet34的转换层
            model.select_head(t)
            model.net2.create_resnet_classifier()
            print(model.net2.res34_cnt)

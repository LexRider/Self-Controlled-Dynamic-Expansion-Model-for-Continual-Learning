...
# training graph attention 2bb，使用大的分类头 + weight
        ...
        while True:
            ...
            if 'class-il' not in model.COMPATIBILITY and 'general-continual' not in model.COMPATIBILITY:
                ...
                if model.args.eval_future and k >= model.current_task:
                    ...
                    feats1 = model.net1(inputs)
                    feats2 = model.net2(inputs)
                    ...
                    if k == 0:
                        out = model.attention_layers[0](feats)
                        outputs = model.classifier_list[0](out)
                    else:
                        weights = torch.softmax(torch.stack(model.weight_list[k]) / model.T, dim=0)
                        out = 0
                        for i in range(k+1):
                            out += weights[i] * model.attention_layers[i](feats)
                        out = model.graph_layers[k](out)  # 通过 graph layer 处理
                        outputs = model.classifier_list[k](out)
...
def train(model: ContinualModel, dataset: ContinualDataset,
    ...
    with track_system_stats(logger) as system_tracker:
        ...
        for t in range(start_task, end_task):
            ...
            # 创建一个新的attention层，将训练参数放进优化器中，创建一个新的学习率调度器
            model.new_attention_layer(train_loader=train_loader)
            model.new_weight(train_loader=train_loader, num_experts=t+1)
            model.new_classifier(train_loader=train_loader, out_dim=dataset.N_CLASSES)
            
            if t == 1:
                model.print_trainable_params(model.attention_opt)
                model.print_trainable_params(model.weight_opt)
                model.print_trainable_params(model.opt)
            ...
            # if t > 0 :
            #     print(f"gate初始结果：{model.gate_list[-1].start_gate_values}")
            #     print(f"gate优化结果：{model.gate_list[-1].end_gate_values}")
            
            print(f"weight list[-1]内容如下：")
            item = model.weight_list[-1]
            # print(torch.softmax(torch.stack(item).detach()  / model.T, dim=0))
            print(item)
            
            # print("attention layers长度：", len(model.attention_layers))
            # print("graph layers长度：", len(model.graph_layers))
            # model.print_optimizer_parameters()
            ...
            model.meta_end_task(dataset)
            end_time = time()
            print("单task运行时间：", end_time - start_time)
        ...
        t_time2 = time()
        print(f"代码运行总用时：{(t_time2 - t_time1)/60} minutes")
        if args.validation:

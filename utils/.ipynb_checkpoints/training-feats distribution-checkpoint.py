...
def save_models(models, save_dir='/hy-tmp/feats_distribution'):
    """
    保存多个模型到本地，支持每个模型自定义文件名

    Args:
        models (dict): 格式为 {'key': (model, filename)} 或 {'key': model}
        save_dir (str): 保存目录
    """
    os.makedirs(save_dir, exist_ok=True)

    for key, value in models.items():
        # 判断是否是 (model, filename) 的形式
        if isinstance(value, tuple):
            model, filename = value
        else:
            model = value
            filename = key  # 默认用字典的key作为文件名

        if not isinstance(model, torch.nn.Module):
            print(f"[警告] {key} 不是有效模型，跳过。")
            continue

        save_path = os.path.join(save_dir, f"{filename}.pth")
        torch.save(model.state_dict(), save_path)
        print(f"[✓] 模型 {key} 已保存为 {save_path}")
...
def evaluate(model: ContinualModel, dataset: ContinualDataset, last=False, return_loss=False) -> Tuple[list, list]:
    ...
    for k, test_loader in enumerate(dataset.test_loaders):
        if k == 4:
            save_models({
                'net1': (model.net1, 'WD_vit_in21k_ft_in1k'),
                'net2': (model.net2, 'WD_vit_in21k')
            })
        ...
        # ⚠️ 仅当是当前任务为任务9时，才初始化用于保存特征的结构
        if model.current_task_num == 4:
            task_feats1 = []
            task_feats2 = []
            task_labels = []
            task_block_feats1 = [[], [], []]
        ...
        while True:
            ...
            if 'class-il' not in model.COMPATIBILITY and 'general-continual' not in model.COMPATIBILITY:
                ...
                if model.args.eval_future and k >= model.current_task:
                    ...
                    feats1, (feats1_1, feats1_2, feats1_3) = model.net1(inputs, returnt='both')
                    feats2, (feats2_1, feats2_2, feats2_3) = model.net2(inputs, returnt='both')

                    # ⚠️ 只在任务9时记录特征
                    if model.current_task_num == 4:
                        task_feats1.append(feats1.cpu())
                        task_feats2.append(feats2.cpu())
                        task_labels.append(labels.cpu())
                        task_block_feats1[0].append(feats1_1.cpu())
                        task_block_feats1[1].append(feats1_2.cpu())
                        task_block_feats1[2].append(feats1_3.cpu())
                        task_block_feats2[0].append(feats2_1.cpu())
                        task_block_feats2[1].append(feats2_2.cpu())
                        task_block_feats2[2].append(feats2_3.cpu())
        ...
        # ⚠️ 如果是任务9，保存整任务特征
        if model.current_task_num == 4:
            task_feats1 = torch.cat(task_feats1, dim=0)
            task_feats2 = torch.cat(task_feats2, dim=0)
            task_labels = torch.cat(task_labels, dim=0)
            task_block_feats1 = [torch.cat(b, dim=0) for b in task_block_feats1]
            task_block_feats2 = [torch.cat(b, dim=0) for b in task_block_feats2]

            os.makedirs('features', exist_ok=True)
            save_path = os.path.join('/hy-tmp/feats_distribution', f'task{k}_features.pt')
            torch.save({
                'net1_feats': task_feats1,
                'net2_feats': task_feats2,
                'net1_blocks': task_block_feats1,
                'net2_blocks': task_block_feats2,
                'labels': task_labels
            }, save_path)
            print(f"[✓] 特征保存成功：{save_path}")
...
#                     if model.current_task_num == 0:
#                         save_vit_features(model, inputs, k)
#                     #################################################################################

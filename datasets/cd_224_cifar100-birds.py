...
class SequentialMutiDomain(ContinualDataset):
    ...
    CIFAR100_TEST_TRANSFORM = transforms.Compose([transforms.Resize(224, interpolation=InterpolationMode.BICUBIC),
                                              # transforms.CenterCrop(224),
                                              transforms.ToTensor(),
                                              transforms.Normalize(CIFAR100_MEAN, CIFAR100_STD)])
    ...
    #     transforms.RandomApply(torch.nn.ModuleList([T.GaussianBlur(kernel_size=3,sigma=(0.2, 5))]),p=0.1),
    #     transforms.RandomHorizontalFlip(),
    ...
    CIFAR10_TEST_TRANSFORM = transforms.Compose([transforms.Resize(224), transforms.ToTensor(), transforms.Normalize(CIFAR10_MEAN, CIFAR100_STD)])
    ...
    def get_data_loaders(self) -> Tuple[DataLoader, DataLoader]:
        ...
        for i in range(len(bird_train_dataset.targets)):
            bird_train_dataset.targets[i] += 100
        for i in range(len(bird_test_dataset.targets)):
            bird_test_dataset.targets[i] += 100

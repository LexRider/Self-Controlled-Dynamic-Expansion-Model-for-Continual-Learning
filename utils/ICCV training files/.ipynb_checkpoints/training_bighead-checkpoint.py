...
# training graph attention 2bb，使用大的分类头
            ...
            if 'class-il' not in model.COMPATIBILITY and 'general-continual' not in model.COMPATIBILITY:
                ...
                if model.args.eval_future and k >= model.current_task:
                    ...
                    if k == 0:
                        ...
                        weights = torch.softmax(torch.stack(model.weight_list[k]).detach() / model.T, dim=0)
                        out = 0
                        for i in range(k + 1):
                            out += weights[i] * model.attention_layers[i](feats)
                        out = model.graph_layers[k](out)
...
def train(model: ContinualModel, dataset: ContinualDataset,
    ...
    with track_system_stats(logger) as system_tracker:
        ...
        for t in range(start_task, end_task):
            ...
            # 更新模型的优化器，创建分类器
            model.update_optimizer(train_loader=train_loader, out_dim=dataset.N_CLASSES)
            ...
            # print(f"当前weight list长度：{len(model.weight_list)}")
            ...
            # model.calculate_qkv_ranks(model.attention_layers[-1], threshold=1)
            # model.calculate_qkv_ranks(model.graph_layers[-1], threshold=1)

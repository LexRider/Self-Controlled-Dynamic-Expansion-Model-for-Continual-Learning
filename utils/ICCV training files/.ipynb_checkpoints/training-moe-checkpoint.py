        ...
        while True:
            ...
            if 'class-il' not in model.COMPATIBILITY and 'general-continual' not in model.COMPATIBILITY:
                ...
                if model.args.eval_future and k >= model.current_task:
                    ...
                    ###############################################################
                    out = model.net(inputs, None, k, is_train=True)
...
###############################################################          
        for t in range(start_task, end_task):
            ...
            ###############################################################  
            model.task_id = t
            model.update_opt_sched_head(train_loader, args.lr, dataset.N_CLASSES)
            ###############################################################  
            model.meta_begin_task(dataset)
            if not args.inference_only and args.n_epochs > 0:
                ...
                while True:
                    ...
                    print(f"单epoch代码执行时间: {epoch_time} 秒")
            ...
            ###############################################################  
            model.net.visual.transformer.freeze_all_blocks_top_k_experts()
            ###############################################################  
            model.meta_end_task(dataset)
...
###############################################################        
        ...
        print(f"代码总执行时间: {execution_time} 秒")
        print(f"代码timelist时间：{time_list}")
###############################################################   
        if args.validation:

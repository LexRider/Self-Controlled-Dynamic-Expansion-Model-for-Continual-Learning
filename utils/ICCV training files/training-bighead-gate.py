            ...
            if 'class-il' not in model.COMPATIBILITY and 'general-continual' not in model.COMPATIBILITY:
                ...
                if model.args.eval_future and k >= model.current_task:
                    ...
                    if k == 0:
                        ...
                        expert_outputs = []
                        for i in range(k + 1):
                            expert_outputs.append(model.attention_layers[i](feats))
                            
                        expert_outputs = torch.stack(expert_outputs)  # 形状 (num_experts, batch_size, feature_dim)
                        gated_output = model.gate_list[k](expert_outputs)
                        out = model.graph_layers[k](gated_output)  # 通过 graph layer 处理
